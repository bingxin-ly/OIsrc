...
void dp(int u, int p)
{
    ...
    for (auto a : graph[u])
    {
        ...
        for (int b = 0; b <= min(size[u], k); b++)
            for (int c = 0; c <= min(size[s], k - b); c++)
            {
                plss(f[u][b + c][0][0], g[b][0][0] * (ssize_t)f[s][c][0][1]);

                plss(f[u][b + c][1][0], g[b][1][0] * ((ssize_t)f[s][c][0][0] + f[s][c][0][1]));

                plss(f[u][b + c][0][1], g[b][0][1] * ((ssize_t)f[s][c][0][1] + f[s][c][1][1]));
                plss(f[u][b + c][0][1], g[b][0][0] * (ssize_t)f[s][c][1][1]);

                plss(f[u][b + c][1][1], g[b][1][0] * ((ssize_t)f[s][c][1][0] + f[s][c][1][1]));
                plss(f[u][b + c][1][1], g[b][1][1] *
                     ((ssize_t)f[s][c][0][0] + f[s][c][0][1] + f[s][c][1][0] + f[s][c][1][1]));
                     // 上面四连加有可能会溢出！！！
            }
        size[u] += size[s];
    }
