...
struct huge : vector<int> // 用标准库vector做基类，完美解决位数问题，同时更易于实现
{
    ...
    inline huge &check() // 在各类运算中经常用到的进位小函数，不妨内置
    {
        while (!back() && !empty())
    ...
    inline huge &operator=(int n)
    {
        while (n)
            base(*this).push_back(n % 10), n /= 10;
        return *this;
    }

    // 输入输出
    friend istream &operator>>(istream &is, huge &n)
    {
        string s;
        is >> s;
        n.clear();
    ...
    friend ostream &operator<<(ostream &os, const huge &n)
    {
        if (n.empty())
            return os << 0;
    ...
    // 常量引用当参数，避免拷贝更高效
    // basic operator == and <
    friend inline bool operator==(const huge &a, const huge &b)
    {
