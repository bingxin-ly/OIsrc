...
{
    top[o] = t, cnt++;
    dfn[o] = cnt, rnk[cnt] = o, ww[cnt] = w[o];
    if (son[o] == -1)
        return;
    dfs2(son[o], t);
    for (int i = head[o]; i; i = nxt[i])
...
class segment_tree
{
    ...
    static void pushup(node *p) { p->val = ((p->ls ? p->ls->val : 0) + (p->rs ? p->rs->val : 0) % mod); }
